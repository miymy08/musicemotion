...
<html>
  ...
  <body>
    ...
    <div class="content">
      <h3>Search result for <?php echo $search_result ?></h3>
      <ul class="w3-ul w3-block songtable">
        <?php
          $query = mysqli_query($con,"SELECT * FROM mp WHERE m_title LIKE '%$search_result%';");
          while($row = mysqli_fetch_array($query)){
            $music_code = $row['m_code'];
            $query_music = mysqli_query($con,"SELECT * FROM mp WHERE m_code = '$music_code'");
            $query_singer = mysqli_query($con,"SELECT * FROM singer WHERE m_code = '$music_code'");
            $singer_array = array();
            while ($row_singer = mysqli_fetch_array($query_singer)){
              array_push($singer_array, $row_singer['s_name']);
            }
            $singer_string = implode(", ",$singer_array);
            $row_m = mysqli_fetch_array($query_music);
        ?>
